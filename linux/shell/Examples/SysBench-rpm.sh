...
finalize_yum_repo ()
{
  if [ "$_skip_pygpgme" = 0 ]; then
    ...
    yum install -y pygpgme --disablerepo='akopytov_sysbench'
    ...
    if [ "$?" != "0" ]; then
      ...
      sed -i'' 's/repo_gpgcheck=1/repo_gpgcheck=0/' /etc/yum.repos.d/akopytov_sysbench.repo
    fi
  fi

  echo "Installing yum-utils..."
  yum install -y yum-utils --disablerepo='akopytov_sysbench'
  ...
  echo "Generating yum cache for akopytov_sysbench..."
  yum -q makecache -y --disablerepo='*' --enablerepo='akopytov_sysbench'

  echo "Generating yum cache for akopytov_sysbench-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='akopytov_sysbench-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh akopytov_sysbench
  zypper --gpg-auto-import-keys refresh akopytov_sysbench-source
}
...
main ()
{
  ...
  yum_repo_config_url="https://packagecloud.io/install/repositories/akopytov/sysbench/config_file.repo?os=${os}&dist=${dist}&source=script"

  if [ "${os}" = "sles" ] || [ "${os}" = "opensuse" ]; then
    yum_repo_path=/etc/zypp/repos.d/akopytov_sysbench.repo
  else
    yum_repo_path=/etc/yum.repos.d/akopytov_sysbench.repo
  fi
